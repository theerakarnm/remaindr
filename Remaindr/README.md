...
```bash
cd Remaindr
xcodebuild -scheme Remaindr -configuration Debug -derivedDataPath ./.build build
open ./.build/Build/Products/Debug/Remaindr.app
```
...
Keys are entered in the app's Settings window and stored in the macOS Keychain under service `com.theerakarn.Remaindr`.
