...
- [x] Run: `/usr/libexec/PlistBuddy -c "Print :LSUIElement" AIUsageBar/.build/Build/Products/Release/AIUsageBar.app/Contents/Info.plist` - Expected: `true`.
- [x] Run: `git status --porcelain AIUsageBar | head` - Expected: no output, meaning every file the plan created is committed and `.build/` is ignored.
- [x] Run: `grep -rn 'sk-' AIUsageBar --include='*.swift' || echo "no key literals"` - Expected: `no key literals`. This is a negative control: the string `sk-` appears nowhere in committed Swift source.
- [ ] Manual: with **no keys saved at all**, launch the Release build and open the dropdown - Expected: all three rows render, the z.ai and DeepSeek rows read `Not configured`, the Claude row shows a real percentage if `~/.claude/projects` has sessions and `Not configured` otherwise. No crash, no zero passed off as data.
      > Unverified: launched the Release build with no keys saved (confirmed via `security find-generic-password` for all three accounts beforehand) and confirmed the process runs as a backgrounded agent with no crash. Could not open the dropdown itself: this automation session has neither Accessibility nor Screen Recording permission (`osascript ... System Events` returned "not allowed assistive access"; `screencapture` returned "could not create image from display"), so the menu bar item cannot be clicked or visually inspected from here. Proxy: Task 9 Step 4's harness asserts the exact row text for this configuration (`all_none="Not set up" symbol=gauge.with.needle`) and Task 8 Step 4 asserts a freshly constructed store's slots are all nil with no crash (`SLOTS=3 ALL_EMPTY=true`).
- [ ] Manual: `defaults read com.theerakarn.AIUsageBar` after saving and clearing a dummy key in Settings - Expected: only `refreshIntervalMinutes`, `labelSource`, `allowBilledClaudeProbe`; no key material anywhere in the output.
      > Unverified: same permission gap as above; saving a key requires typing into the Settings window's SecureField, which this session cannot drive. Proxy: Task 3 Step 2 proves the exact `KeychainStore.set`/`value`/`remove` round trip `SettingsView.save`/`clear` call directly, and Task 3 Step 3 proves a saved value never reaches `UserDefaults`.
- [ ] Manual: `security find-generic-password -s com.theerakarn.AIUsageBar -a deepseek -g 2>&1 | head -3` after saving a dummy DeepSeek key - Expected: the item is found under that service and account, proving the key landed in the Keychain and not in a file.
      > Unverified: same permission gap; same proxy (Task 3 Step 2's `READBACK=sk-verify-123` round trip against the real Keychain, from an unsigned binary with no GUI prompt).
- [ ] Manual: quit the app, relaunch it, and reopen Settings - Expected: the DeepSeek field shows the `Key saved` placeholder, proving the Keychain value survived a restart. Then clear it.
      > Unverified: same permission gap; `SettingsView.onAppear` computes `savedKinds` from `keychain.hasKey(for:)`, which Task 3 Step 2's `HAS=true false` line already proves reads back correctly after a set.
- [ ] 👤 Human: with all three providers configured, look at the collapsed menu bar item beside the battery and clock - Expected: the label stays narrow enough that no existing menu bar item is pushed off screen, at every value the label can take - Proxy: Task 9 Step 4 asserts `MAX_LEN=14 BUDGET=14 WITHIN=true` across ten label states, including all three providers at once, a seven-figure balance, and an unknown currency code that forces the clamp.
      > Awaiting human: proxy passed (see Task 9 Step 4); the visual judgement itself is the user's to make.
- [ ] 👤 Human: turn Wi-Fi off, click Refresh, and watch a previously-populated provider row - Expected: the row keeps its previous figure and gains an `Offline` indicator; it does not blank and does not show 0 - Proxy: Task 8 Step 4 asserts `STALE_KEPT=true ERROR=Offline` on the same code path, and Task 4 Step 2 asserts the parse layer throws rather than returning a zeroed status.
      > Awaiting human: proxy passed (see Task 8 Step 4 and Task 4 Step 2); the live network-loss check is the user's to run.
- [ ] 👤 Human: copy the Release build to `/Applications`, launch it, and toggle Launch at login - Expected: the toggle sticks and the app appears under System Settings, General, Login Items - Proxy: Task 10 Step 4 proves `LoginItem` compiles against `SMAppService` at the 14.0 deployment target; the toggle cannot be exercised from a build directory, which the Failure handling section records.
      > Awaiting human: proxy passed (see Task 10 Step 4); copying to /Applications and toggling the login item is the user's to do.
