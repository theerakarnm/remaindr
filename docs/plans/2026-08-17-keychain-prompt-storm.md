...
**6. Two facts about the mechanism, from Apple's own Security sources, that the fixes rest on.**
The file-based keychain's default ACL grants `ENCRYPT` to any application and `DECRYPT` only to the trusted list (`OSX/libsecurity_keychain/lib/Access.cpp:83-110`), and attributes are plaintext columns, which is why only a data read is gated.
Every prompt in the log is `AclValidationContext(action:24)` and `CSSM_WORDID_DECRYPT` is 24; the extra password field belongs to action `65538`, `CSSM_ACL_AUTHORIZATION_PARTITION_ID`, which is the "Always Allow" write.
`ItemImpl::updateSSGroup` (`OSX/libsecurity_keychain/lib/Item.cpp:924-941`) copies the existing ACL on the update path and builds a brand new one trusting only the adding process on the add path, which is exactly why Task 3 replaces delete-then-add with `SecItemUpdate`.
