...
- [Working tree clean on main at 090d214 plus this plan document] - Check: `git status -sb` is clean and `git log --oneline 090d214..HEAD` lists only commits touching `docs/plans/2026-08-18-github-actions-ci.md` (planning run: clean; the plan file's own two commits are expected on top of 090d214, no source drift) - Needed by: all tasks. If any non-plan-doc commit appears in that range, diff the plan's anchors before editing.
